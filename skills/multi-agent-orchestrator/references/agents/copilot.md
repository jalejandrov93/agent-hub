# copilot (GitHub Copilot CLI) — reference
...
## Models — only `auto` is verified reliable on this account
...
**That catalog is documentation, not account availability.** Live testing on this account
rejected every explicit id tried (`gpt-5-mini`, `gpt-4.1`, `gpt-5.4-mini`, `gpt-5.3-codex`,
`gpt-5.4`, `claude-sonnet-4.6`, `claude-haiku-4.5`, `gpt-5.6-luna`) with:

```
Error: Model "<id>" from --model flag is not available.
```

written to stderr, process exit varied (0 observed in one recording, 1 in another — the adapter
matches on message text, not exit code, so this is moot for parsing). Only `--model auto`
worked; it silently resolved to `gpt-5.6-luna` — an id that does not even appear in
`help config`. **Route to `auto` for every task type; treat any explicit id as
`degraded`/unverified until an L3 ping confirms it (`agent-hub preflight --agent copilot
--model <id> --ping`).**

copilot auto-updates its own binary (1.0.31 → 1.0.83 observed mid-session) — do not assume a
pinned CLI version.
...
- Quota: `gh api /copilot_internal/user` → `quota_snapshots.premium_interactions
  .percent_remaining` — advisory; billing on this account moved to AI credits, so this number
  may stop reflecting real remaining capacity. Do not gate delegation on it, only inform.

## Gotchas (measured)

1. Never pass an explicit `--model` id expecting it to work on this account — use `auto`.
